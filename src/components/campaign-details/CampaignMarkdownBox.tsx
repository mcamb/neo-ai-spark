...
interface CampaignMarkdownBoxProps {
...
export const CampaignMarkdownBox: React.FC<CampaignMarkdownBoxProps> = ({ 
  children, 
  isEditing = false, 
  onEdit, 
  value,
  ...
  if (isEditing && onEdit) {
    return (
      <div className="relative h-full">
        <Textarea 
          className={`min-h-[120px] w-full h-full border border-gray-300 rounded-lg focus:border-[#FF4B4F] text-left ${className || ''}`}
          value={value} 
          onChange={(e) => onEdit(e.target.value)}
          style={{ ...style, height: '100%' }}
        />
        <div className="text-xs text-black mt-2 text-left">
  ...
  const boxClassName = `p-5 border border-gray-200 rounded-lg max-w-none text-left ${
    darkMode && !isEditing ? 'bg-[#363636]' : 'bg-white'
  } ${className || ''}`;
