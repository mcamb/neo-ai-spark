  ...
  useEffect(() => {
    const pollingInterval = setInterval(() => {
      ...
      if (inProgressClients.length > 0 && Math.random() > 0.7) {
        ...
        setClients(prev => prev.map(client => client.id === randomClient.id ? {
          ...client,
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Type here to search" className="pl-10 border-gray-300 focus:border-neo-red focus:ring-neo-red" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          </div>
          <Button onClick={() => setIsModalOpen(true)} className="bg-neo-red hover:bg-red-600 text-white">
            Add client
          </Button>
        </div>
        
        <div className="bg-white rounded-lg shadow-sm p-1">
          <ClientsList clients={clients} searchQuery={searchQuery} countryNames={countryNames} onDeleteClient={handleDeleteClient} />
        </div>
      </div>
      
      <NewClientModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSubmit={handleAddClient} />
    </MainLayout>;
};
